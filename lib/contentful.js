...
export async function getSiteInfo() {
  ...
  try {
    ...
    return {
      ...
      availabilityColor: fields.availabilityColor || FALLBACK_SITE_INFO.availabilityColor,
...
export async function getWorkImages() {
  ...
  try {
    ...
    return entries.items.map((item) => {
      ...
      // Resolve the linked project reference (if set)
      const linkedProject = item.fields.linkedProject;
      let project = null;
      if (linkedProject?.fields) {
        project = {
          title: linkedProject.fields.title || '',
          description: linkedProject.fields.description || '',
          url: linkedProject.fields.url || '',
          isExternal: linkedProject.fields.isExternal || false,
          isWip: linkedProject.fields.isWip || false,
        };
      }
